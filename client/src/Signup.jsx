  ...
  const validateField = (fieldName, value) => {
    ...
    if (fieldName === 'username') {
      if (notValidUsername(value)) {
        newFieldErrors.push('Usernames must only include underscores and lowercase alphanumerical characters (a-z) and be at least 3 characters in length.');
      }
    }
        ...
        <Grid container direction="column">
          <Grid item p={2}>
            <TextField
              ...
              type="text"
              name="username"
              error={(error.username.length > 0) || serverError !== undefined}
              helperText={error.username.join(', ') || serverError}
            />
            <TextField
              margin={"normal"}
              fullWidth
